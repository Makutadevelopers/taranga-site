...
modal. Both currently use the same approved template, `taranga_website_1` (approved
2026-07-14; it replaced `taranga_website`).

| Env var | Value | Purpose |
|---|---|---|
| `WA_TPL_BROCHURE` | `taranga_website_1` | template sent on a brochure enquiry |
| `WA_TPL_PRICE` | `taranga_website_1` | template sent on a price-sheet enquiry; falls back to `WA_TPL_BROCHURE` if unset |
| `WA_IMG_BROCHURE` / `WA_IMG_PRICE` | _(unset)_ | header image URL — **only** set these if the template actually has an image header |

- **Body params:** `{{1}}` = customer name (`Hello [Name],`). One param, nothing else.
- **Header:** none. The message opens straight into body text — so `WA_IMG_*` stay unset.
- **Buttons:** Taranga Brochure / Taranga Floor Plans / Taranga Price Sheet. These carry no
  parameters, so `whatsapp.js` sends no button component. If they are **quick replies**, a
  tap only sends a message back to the business number — it does **not** deliver a file, and
  an api-wa.co auto-reply flow (or a human) has to respond with the PDF.
