// Auto-generated from about.html <style> blocks (verbatim). Rendered mount-scoped.
const css = `:root{
 --navh:62px;
...
.proj .pmeta{position:absolute;left:0;right:0;bottom:0;z-index:2;padding:1.3rem 1.4rem;color:#F7F4ED}
.proj .pstatus{font-size:.58rem;letter-spacing:.24em;text-transform:uppercase;color:var(--line)}
.proj .pname{font-family:var(--display);font-weight:400;font-size:1.5rem;line-height:1.05;margin:.3rem 0 .5rem}
.proj .plink{font-size:.62rem;letter-spacing:.16em;text-transform:uppercase;display:inline-flex;gap:.5rem;align-items:center;opacity:.92}
.proj:hover .plink{gap:.8rem;color:var(--line)}

/* about — Crown of Excellence values */
.vals{display:grid;grid-template-columns:repeat(3,1fr);gap:clamp(1.3rem,2.8vw,2.2rem);margin-top:.2rem}
.val{padding:1.3rem 0 0;border-top:2px solid var(--line)}
.vh{font-family:var(--display);font-weight:400;font-size:1.32rem;color:var(--ink);line-height:1.15;margin-bottom:.5rem}
.val p{color:var(--ink-soft);font-size:.92rem;line-height:1.55;margin:0}
@media(max-width:760px){.vals{grid-template-columns:1fr;gap:1.3rem}.val{padding-top:1.05rem}}
...
.testimonials-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:clamp(1.4rem,3vw,2.2rem);margin-top:.5rem}
.test-card{cursor:pointer;border-radius:4px;overflow:hidden;
  box-shadow:0 8px 24px rgba(14,27,44,.12);transition:transform .3s var(--ease),box-shadow .3s var(--ease);}
.test-card:hover{transform:translateY(-8px);box-shadow:0 20px 50px rgba(14,27,44,.2)}
.test-video{position:relative;width:100%;aspect-ratio:1;background-size:cover;
  background-position:center;background-color:#ddd;display:flex;align-items:center;justify-content:center}
.test-play{font-size:3rem;color:#F7F4ED;text-shadow:0 2px 10px rgba(14,27,44,.4);
  transition:transform .3s var(--ease)}
.test-card:hover .test-play{transform:scale(1.15)}
.test-meta{padding:1.3rem 1.2rem;background:#fff;border-top:2px solid var(--line)}
.test-name{font-family:var(--display);font-weight:400;font-size:1.1rem;color:var(--ink);margin-bottom:.2rem}
